...
<div>
  ...
  <el-table :data="tableData" border stripe :header-cell-class-name="'headerBg'" row-key="id" default-expand-all @selection-change="handleSelectionChange">
    ...
    <el-table-column label="操作" width="300" align="center">
      <template  slot-scope="scope">
        <el-button type="primary" @click="addchildren(scope.row.id)" v-if="!scope.row.pid && !scope.row.path">分配菜单<i class="el-icon-plus"></i></el-button>
  ...
  <el-dialog title="菜单信息" :visible.sync="dialogFormVisible" width="27%">
...
export default {
  ...
  methods:{
    ...
    // },
    save(){
      this.request.post("/menus",this.form).then(res=>{
        if(res.statue == 200){
    ...
    handleDelete(id){
      this.request.delete("/menus/"+id).then(res=>{
        if(res.statue == 200){
    ...
    deleteBatch(){
      ...
      this.request.post("/menus/batch",ids).then(res=>{
        if(res.statue == 200){
          this.$message.success("删除成功")
          this.load()
        }else {
